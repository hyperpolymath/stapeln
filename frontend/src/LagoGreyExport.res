...
let generateDockerfile = (state: lagoGreyState): string => {
  ...
  let header = "# SPDX-License-Identifier: PMPL-1.0-or-later\n"
    ...
    ++ "# Generated at: " ++ Js.Date.make()->Js.Date.toISOString ++ "\n\n"
  ...
  let formations = state.formations
    ->Array.map(formation => {
      ...
      | _ =>
          "# Add " ++ name ++ " (" ++ Int.toString(size) ++ " bytes)\n"
  ...
  let metadata = "# Image metadata\n"
    ...
    ++ "LABEL com.hyperpolymath.lago-grey.size=\"" ++ Int.toString(state.totalSize) ++ "\"\n\n"
...
let generateManifest = (state: lagoGreyState): string => {
  let formations = state.formations
    ->Array.map(formation => {
      ...
      ++ "      \"size\": " ++ Int.toString(formationSize(formation)) ++ "\n"
      ++ "    }"
    })
    ->Array.joinWith(",\n")
  ...
  ++ "  \"base\": \"" ++ baseStr ++ "\",\n"
  ++ "  \"totalSize\": " ++ Int.toString(state.totalSize) ++ ",\n"
  ++ "  \"formations\": [\n"
  ++ formations ++ "\n"
  ++ "  ],\n"
  ++ "  \"security\": {\n"
  ++ "    \"postQuantum\": " ++ (Array.some(state.formations, f => formationName(f) == "liboqs") ? "true" : "false") ++ ",\n"
  ++ "    \"classical\": " ++ (Array.some(state.formations, f => formationName(f) == "libsodium") ? "true" : "false") ++ "\n"
  ++ "  },\n"
  ++ "  \"signatures\": {\n"
  ++ "    \"dilithium5\": \"TODO\",\n"
  ++ "    \"ed448\": \"TODO\",\n"
...
let exportDockerfile = (state: lagoGreyState) => {
  ...
  let timestamp = Js.Date.now()->Float.toString->String.slice(~start=0, ~end=10)
  let filename = "Dockerfile.lago-grey-" ++ timestamp

  Export.downloadFile(filename, dockerfile, "text/plain")
  Js.Console.log("Exported Dockerfile: " ++ filename)
}
...
let exportManifest = (state: lagoGreyState) => {
  ...
  Js.Console.log("Exported manifest: " ++ filename)
}
...
let exportBuildInstructions = () => {
  ...
  Js.Console.log("Exported build instructions")
}
...
let exportCompletePackage = (state: lagoGreyState) => {
  ...
  Js.Console.log("Exported complete Lago Grey package")
  Js.Console.log("Next: Run 'podman build' to create the image")
}
