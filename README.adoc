...
**stackur** (Icelandic: "stack" + designer) is a visual web application for designing multi-container stacks using the https://github.com/hyperpolymath/verified-container-spec[verified-container-spec] ecosystem. Drag components onto a canvas, connect them, configure them, and stackur generates `compose.toml` files for https://github.com/hyperpolymath/selur-compose[selur-compose] deployment.

=== Key Features

* 🎨 **Drag-and-Drop UI** - Intuitive visual stack designer
* 🔗 **Component Library** - Pre-built components (Cerro Torre, Svalinn, selur, Vörðr)
* ✅ **Formal Validation** - Stack correctness verified via Idris2 proofs
* 📝 **Multi-Format Export** - Generate `compose.toml`, `docker-compose.yml`, `podman-compose.yml`
* 🐳 **Runtime Support** - Podman, Docker, nerdctl
* 🔒 **Type-Safe** - ReScript frontend, Elixir backend, Ephapax validation
...
* **Vörðr** - Container runtime/orchestrator
...
1. Open stackur in browser: `http://localhost:8000`
...
└──────┬───────┘
       │
┌──────▼───────┐
│  Cerro Torre │ .ctp bundle creation
└──────┬───────┘
       │
┌──────▼───────┐
│   Svalinn    │ Edge gateway (auth, policy)
└──────┬───────┘
       │
┌──────▼───────┐
...
│   │   ├── Model.res     # TEA Model
│   │   ├── Msg.res       # TEA Messages
│   │   ├── Update.res    # TEA Update
│   │   ├── View.res      # TEA View
│   │   ├── Canvas.res    # Drag-and-drop canvas
│   │   ├── Components.res # Component library
│   │   └── Router.res    # cadre-tea-router
│   ├── deno.json
│   └── import_map.json
├── backend/               # Elixir Phoenix
│   ├── lib/
│   │   ├── stackur_web/  # Phoenix controllers/channels
│   │   ├── stackur/      # Business logic
│   │   └── validator/    # Ephapax validation
│   ├── native/           # Rust NIFs (codegen)
│   └── mix.exs
├── validation/            # Idris2 proofs
│   └── src/
│       ├── Stack.idr     # Stack type definitions
...
stackur uses **formal verification** to ensure stack correctness:
